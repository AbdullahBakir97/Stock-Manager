...
_TR: dict[str, dict[str, str]] = {
    ...
    "admin_tab_cloud_sync":     {"EN": "Cloud Sync",              "DE": "Cloud-Sync",                  "AR": "مزامنة السحابة"},
    "nav_phones":               {"EN": "Phones",                  "DE": "Telefone",                    "AR": "الهواتف"},
    "nav_logs":                 {"EN": "Logs",                    "DE": "Protokolle",                  "AR": "السجلات"},

    # ── In-app logs viewer ─────────────────────────────────────────────────────
    "log_title":                {"EN": "Application Logs",        "DE": "Anwendungsprotokolle",        "AR": "سجلات التطبيق"},
    "log_window_title":         {"EN": "Stock Manager Pro — Logs", "DE": "Stock Manager Pro — Protokolle", "AR": "Stock Manager Pro — السجلات"},
    "log_popout":               {"EN": "Pop out ⧉",               "DE": "Abdocken ⧉",                  "AR": "نافذة منفصلة ⧉"},
    "log_lvl_all":              {"EN": "All levels",              "DE": "Alle Stufen",                 "AR": "كل المستويات"},
    "log_lvl_debug":            {"EN": "Debug",                   "DE": "Debug",                       "AR": "تصحيح"},
    "log_lvl_info":             {"EN": "Info",                    "DE": "Info",                        "AR": "معلومات"},
    "log_lvl_warning":          {"EN": "Warning",                 "DE": "Warnung",                     "AR": "تحذير"},
    "log_lvl_error":            {"EN": "Error",                   "DE": "Fehler",                      "AR": "خطأ"},
    "log_all_sources":          {"EN": "All sources",             "DE": "Alle Quellen",                "AR": "كل المصادر"},
    "log_search_placeholder":   {"EN": "Search messages…",        "DE": "Meldungen suchen…",           "AR": "بحث في الرسائل…"},
    "log_verbose":              {"EN": "Verbose",                 "DE": "Ausführlich",                 "AR": "مفصّل"},
    "log_verbose_tip":          {"EN": "Capture DEBUG-level detail while troubleshooting",
                                 "DE": "DEBUG-Details während der Fehlersuche erfassen",
                                 "AR": "التقاط تفاصيل التصحيح أثناء استكشاف الأخطاء"},
    "log_pause":                {"EN": "Pause",                   "DE": "Pause",                       "AR": "إيقاف مؤقت"},
    "log_resume":               {"EN": "Resume",                  "DE": "Fortsetzen",                  "AR": "استئناف"},
    "log_col_time":             {"EN": "Time",                    "DE": "Zeit",                        "AR": "الوقت"},
    "log_col_level":            {"EN": "Level",                   "DE": "Stufe",                       "AR": "المستوى"},
    "log_col_source":           {"EN": "Source",                  "DE": "Quelle",                      "AR": "المصدر"},
    "log_col_message":          {"EN": "Message",                 "DE": "Meldung",                     "AR": "الرسالة"},
    "log_clear":                {"EN": "Clear",                   "DE": "Leeren",                      "AR": "مسح"},
    "log_copy":                 {"EN": "Copy",                    "DE": "Kopieren",                    "AR": "نسخ"},
    "log_export":               {"EN": "Export…",                 "DE": "Exportieren…",                "AR": "تصدير…"},
    "log_open_file":            {"EN": "Open log file",           "DE": "Protokolldatei öffnen",       "AR": "فتح ملف السجل"},
    "log_open_folder":          {"EN": "Open folder",             "DE": "Ordner öffnen",               "AR": "فتح المجلد"},
    "log_count":                {"EN": "{shown} of {total} shown", "DE": "{shown} von {total} angezeigt", "AR": "عرض {shown} من {total}"},
    "log_never":                {"EN": "never",                   "DE": "nie",                         "AR": "أبداً"},
    "log_sync_ok":              {"EN": "Cloud sync OK · last synced {time}",
                                 "DE": "Cloud-Sync OK · zuletzt {time}",
                                 "AR": "المزامنة السحابية جيدة · آخر مزامنة {time}"},
    "log_sync_error":           {"EN": "Sync error at {time}: {error}",
                                 "DE": "Sync-Fehler um {time}: {error}",
                                 "AR": "خطأ مزامنة في {time}: {error}"},
    "log_sync_running":         {"EN": "Cloud sync in progress…", "DE": "Cloud-Sync läuft…",        "AR": "جارٍ المزامنة السحابية…"},
    "log_sync_now":             {"EN": "Sync now",                "DE": "Jetzt synchronisieren",       "AR": "مزامنة الآن"},
    "log_sync_only":            {"EN": "Cloud sync only",         "DE": "Nur Cloud-Sync",              "AR": "المزامنة السحابية فقط"},
    "log_sync_detail":          {"EN": "{mode} · {role} · {host}", "DE": "{mode} · {role} · {host}",   "AR": "{mode} · {role} · {host}"},
    "log_sync_mode_embedded":   {"EN": "Embedded replica",        "DE": "Eingebettetes Replikat",      "AR": "نسخة مدمجة"},
    "log_sync_mode_http":       {"EN": "HTTP API",                "DE": "HTTP-API",                    "AR": "واجهة HTTP"},
    "log_sync_mode_local":      {"EN": "Local + push",            "DE": "Lokal + Push",                "AR": "محلي + دفع"},
    "log_sync_role_primary":    {"EN": "Primary",                 "DE": "Primär",                      "AR": "أساسي"},
    "log_sync_role_replica":    {"EN": "Replica",                 "DE": "Replikat",                    "AR": "نسخة"},
    "log_health_tip":           {"EN": "Application health — click to open logs",
                                 "DE": "Anwendungszustand — zum Öffnen der Protokolle klicken",
                                 "AR": "حالة التطبيق — انقر لفتح السجلات"},
    "log_toast_error":          {"EN": "Error in {source}: {message}",
                                 "DE": "Fehler in {source}: {message}",
                                 "AR": "خطأ في {source}: {message}"},
    "log_toast_view":           {"EN": "View logs",               "DE": "Protokolle",                  "AR": "عرض السجلات"},
    "log_row_tip":              {"EN": "Double-click a row to see full details / traceback",
                                 "DE": "Doppelklick auf eine Zeile für Details / Traceback",
                                 "AR": "انقر مزدوجاً على صف لعرض التفاصيل الكاملة"},
    "log_detail_title":         {"EN": "Log entry detail",        "DE": "Protokolldetails",            "AR": "تفاصيل السجل"},
    ...
    "msg_delete_blocked": {
        "EN": "This product can't be deleted — it has sales or stock-movement history. Set its stock to 0 instead.",
        "DE": "Dieses Produkt kann nicht gelöscht werden — es hat Verkaufs- oder Bestandsbewegungshistorie. Setzen Sie stattdessen den Bestand auf 0.",
        "AR": "لا يمكن حذف هذا المنتج — لديه سجل مبيعات أو حركة مخزون. اضبط مخزونه على 0 بدلاً من ذلك.",
    },
    ...
    # ── Phones page ──────────────────────────────────────────────────────────
    "ph_kpi_total":            {"EN": "Total",        "DE": "Gesamt",        "AR": "الإجمالي"},
    "ph_kpi_in_stock":         {"EN": "In Stock",     "DE": "Auf Lager",     "AR": "متوفر"},
    "ph_kpi_sold":             {"EN": "Sold",         "DE": "Verkauft",      "AR": "مباع"},
    "ph_kpi_avg_battery":      {"EN": "Avg Battery",  "DE": "Akku Ø",        "AR": "متوسط البطارية"},
    "ph_kpi_stock_value":      {"EN": "Stock Value",  "DE": "Lagerwert",     "AR": "قيمة المخزون"},
    "ph_filter_all_brands":    {"EN": "All Brands",   "DE": "Alle Marken",   "AR": "كل الماركات"},
    "ph_filter_all_storage":   {"EN": "All Storage",  "DE": "Alle Speicher", "AR": "كل السعات"},
    "ph_filter_all_conditions":{"EN": "All Conditions","DE": "Alle Zustände","AR": "كل الحالات"},
    "ph_cond_new":             {"EN": "New",          "DE": "Neu",           "AR": "جديد"},
    "ph_cond_used":            {"EN": "Used",         "DE": "Gebraucht",     "AR": "مستعمل"},
    "ph_cond_refurbished":     {"EN": "Refurbished",  "DE": "Generalüberholt","AR": "مجدد"},
    "ph_cond_refurb_short":    {"EN": "Refurb",       "DE": "Ref.",          "AR": "مجدد"},
    "ph_search_placeholder":   {"EN": "Search IMEI, model…", "DE": "IMEI, Modell suchen…", "AR": "بحث عن IMEI أو الموديل…"},
    "ph_btn_add":              {"EN": "+ Add Phone",  "DE": "+ Telefon hinzufügen", "AR": "+ إضافة هاتف"},
    "ph_btn_sold_history":     {"EN": "📜 Sold History", "DE": "📜 Verkaufshistorie", "AR": "📜 سجل المبيعات"},
    "ph_tip_sold_history":     {"EN": "View history of sold phones", "DE": "Verlauf der verkauften Telefone anzeigen", "AR": "عرض سجل الهواتف المباعة"},
    "ph_btn_labels":           {"EN": "🏷 Labels",    "DE": "🏷 Etiketten",  "AR": "🏷 ملصقات"},
    "ph_tip_labels":           {"EN": "Export barcode labels for YunPrint", "DE": "Barcode-Etiketten für YunPrint exportieren", "AR": "تصدير ملصقات الباركود لطابعة YunPrint"},
    "ph_btn_refresh":          {"EN": "↺ Refresh",    "DE": "↺ Aktualisieren", "AR": "↺ تحديث"},
    "ph_col_brand_model":      {"EN": "Brand / Model","DE": "Marke / Modell","AR": "الماركة / الموديل"},
    "ph_col_total":            {"EN": "Total",        "DE": "Gesamt",        "AR": "الإجمالي"},
    "ph_units_title":          {"EN": "Units",        "DE": "Einheiten",     "AR": "الوحدات"},
    "ph_units_for":            {"EN": "Units — {model}", "DE": "Einheiten — {model}", "AR": "الوحدات — {model}"},
    "ph_search_results":       {"EN": "Search Results","DE": "Suchergebnisse","AR": "نتائج البحث"},
    "ph_col_imei":             {"EN": "IMEI",         "DE": "IMEI",          "AR": "IMEI"},
    "ph_col_storage":          {"EN": "Storage",      "DE": "Speicher",      "AR": "السعة"},
    "ph_col_condition":        {"EN": "Condition",    "DE": "Zustand",       "AR": "الحالة"},
    "ph_col_battery":          {"EN": "Battery",      "DE": "Akku",          "AR": "البطارية"},
    "ph_col_buy":              {"EN": "Buy",          "DE": "Einkauf",       "AR": "الشراء"},
    "ph_col_sell":             {"EN": "Sell",         "DE": "Verkauf",       "AR": "البيع"},
    "ph_col_status":           {"EN": "Status",       "DE": "Status",        "AR": "الحالة"},
    "ph_col_notes":            {"EN": "Notes",        "DE": "Notizen",       "AR": "ملاحظات"},
    "ph_col_actions":          {"EN": "Actions",      "DE": "Aktionen",      "AR": "إجراءات"},
    "ph_tip_edit":             {"EN": "Edit",         "DE": "Bearbeiten",    "AR": "تعديل"},
    "ph_tip_delete":           {"EN": "Delete",       "DE": "Löschen",       "AR": "حذف"},
    "ph_status_in_stock":      {"EN": "In Stock",     "DE": "Auf Lager",     "AR": "متوفر"},
    "ph_status_sold":          {"EN": "Sold",         "DE": "Verkauft",      "AR": "مباع"},
    "ph_status_reserved":      {"EN": "Reserved",     "DE": "Reserviert",    "AR": "محجوز"},
    "ph_delete_title":         {"EN": "Delete Phone", "DE": "Telefon löschen", "AR": "حذف الهاتف"},
    "ph_delete_confirm":       {
        "EN": "Delete {name}?\n\nThis action cannot be undone (use Undo button to restore).",
        "DE": "{name} löschen?\n\nDiese Aktion kann nicht rückgängig gemacht werden (Wiederherstellung über die Rückgängig-Taste möglich).",
        "AR": "حذف {name}؟\n\nلا يمكن التراجع عن هذا الإجراء (استخدم زر التراجع للاستعادة).",
    },
    "ph_delete_error_title":   {"EN": "Delete Error", "DE": "Fehler beim Löschen", "AR": "خطأ في الحذف"},
    "ph_undo_add":             {"EN": "Add {name}",   "DE": "{name} hinzufügen", "AR": "إضافة {name}"},
    "ph_undo_edit":            {"EN": "Edit {name}",  "DE": "{name} bearbeiten", "AR": "تعديل {name}"},
    "ph_undo_delete":          {"EN": "Delete {name}","DE": "{name} löschen", "AR": "حذف {name}"},
    "ph_undo_status":          {"EN": "{name} → {status}", "DE": "{name} → {status}", "AR": "{name} ← {status}"},

    # ── Add/Edit phone dialog ────────────────────────────────────────────────
    "phd_title_add":           {"EN": "Add Phone",    "DE": "Telefon hinzufügen", "AR": "إضافة هاتف"},
    "phd_title_edit":          {"EN": "Edit Phone",   "DE": "Telefon bearbeiten", "AR": "تعديل الهاتف"},
    "phd_lbl_model":           {"EN": "Model *",      "DE": "Modell *",      "AR": "الموديل *"},
    "phd_lbl_imei":            {"EN": "IMEI",         "DE": "IMEI",          "AR": "IMEI"},
    "phd_imei_placeholder":    {"EN": "Optional — leave blank if unknown", "DE": "Optional — leer lassen, falls unbekannt", "AR": "اختياري — اتركه فارغًا إذا كان غير معروف"},
    "phd_lbl_storage":         {"EN": "Storage",      "DE": "Speicher",      "AR": "السعة"},
    "phd_lbl_condition":       {"EN": "Condition",    "DE": "Zustand",       "AR": "الحالة"},
    "phd_lbl_battery":         {"EN": "Battery",      "DE": "Akku",          "AR": "البطارية"},
    "phd_unknown":             {"EN": "Unknown",      "DE": "Unbekannt",     "AR": "غير معروف"},
    "phd_lbl_buy":             {"EN": "Buy Price",    "DE": "Einkaufspreis", "AR": "سعر الشراء"},
    "phd_lbl_sell":            {"EN": "Sell Price",   "DE": "Verkaufspreis", "AR": "سعر البيع"},
    "phd_lbl_status":          {"EN": "Status",       "DE": "Status",        "AR": "الحالة"},
    "phd_lbl_notes":           {"EN": "Notes",        "DE": "Notizen",       "AR": "ملاحظات"},
    "phd_notes_placeholder":   {"EN": "Optional notes…", "DE": "Optionale Notizen…", "AR": "ملاحظات اختيارية…"},
    "phd_validation_title":    {"EN": "Validation",   "DE": "Validierung",   "AR": "تحقق"},
    "phd_validation_model":    {"EN": "Please select a valid phone model.", "DE": "Bitte wählen Sie ein gültiges Telefonmodell aus.", "AR": "يرجى اختيار موديل هاتف صالح."},
    "phd_dup_imei_title":      {"EN": "Duplicate IMEI", "DE": "Doppelte IMEI", "AR": "IMEI مكرر"},
    "phd_dup_imei_body":       {
        "EN": "IMEI {imei} already exists in the database.\n\nSave anyway?",
        "DE": "Die IMEI {imei} existiert bereits in der Datenbank.\n\nTrotzdem speichern?",
        "AR": "رقم IMEI {imei} موجود بالفعل في قاعدة البيانات.\n\nهل تريد الحفظ على أي حال؟",
    },
    "phd_save_error_title":    {"EN": "Save Error",   "DE": "Fehler beim Speichern", "AR": "خطأ في الحفظ"},

    # ── Phone scan action dialog ─────────────────────────────────────────────
    "phs_title":               {"EN": "Scanned Phone Unit", "DE": "Gescanntes Telefon", "AR": "هاتف تم مسحه"},
    "phs_badge_in_stock":      {"EN": "IN STOCK",     "DE": "AUF LAGER",     "AR": "متوفر"},
    "phs_badge_sold":          {"EN": "SOLD",         "DE": "VERKAUFT",      "AR": "مباع"},
    "phs_badge_reserved":      {"EN": "RESERVED",     "DE": "RESERVIERT",    "AR": "محجوز"},
    "phs_stat_storage":        {"EN": "Storage",      "DE": "Speicher",      "AR": "السعة"},
    "phs_stat_condition":      {"EN": "Condition",    "DE": "Zustand",       "AR": "الحالة"},
    "phs_stat_battery":        {"EN": "Battery",      "DE": "Akku",          "AR": "البطارية"},
    "phs_stat_sell_price":     {"EN": "Sell Price",   "DE": "Verkaufspreis", "AR": "سعر البيع"},
    "phs_btn_sold":            {"EN": "✓ Stock OUT  (Mark Sold)", "DE": "✓ Warenausgang  (Als verkauft markieren)", "AR": "✓ إخراج من المخزون (وضع علامة مباع)"},
    "phs_btn_reserve":         {"EN": "⏸ Reserve",    "DE": "⏸ Reservieren", "AR": "⏸ حجز"},
    "phs_btn_back":            {"EN": "↩ Back to Stock", "DE": "↩ Zurück auf Lager", "AR": "↩ إعادة إلى المخزون"},
    "phs_btn_edit":            {"EN": "✎ Edit",       "DE": "✎ Bearbeiten",  "AR": "✎ تعديل"},
    "phs_btn_view":            {"EN": "↗ View in Phones", "DE": "↗ In Telefone anzeigen", "AR": "↗ عرض في الهواتف"},
    "phs_btn_cancel":          {"EN": "Cancel",       "DE": "Abbrechen",     "AR": "إلغاء"},
    "phs_footer_note":         {
        "EN": "ℹ  Each phone unit is 1 unique device (IMEI-tracked). Stock OUT = Mark Sold.  Add new units via the Phones page.",
        "DE": "ℹ  Jede Telefoneinheit ist ein eindeutiges Gerät (IMEI-erfasst). Warenausgang = Als verkauft markieren. Neue Einheiten über die Telefone-Seite hinzufügen.",
        "AR": "ℹ  كل وحدة هاتف هي جهاز فريد (متتبع برقم IMEI). الإخراج من المخزون = وضع علامة مباع. أضف وحدات جديدة من صفحة الهواتف.",
    },

    # ── Sold phones history dialog ───────────────────────────────────────────
    "phh_title":               {"EN": "Sold Phones History", "DE": "Verkaufshistorie der Telefone", "AR": "سجل الهواتف المباعة"},
    "phh_search_placeholder":  {"EN": "Search IMEI, brand, model…", "DE": "IMEI, Marke, Modell suchen…", "AR": "بحث عن IMEI أو الماركة أو الموديل…"},
    "phh_filter_by_date":      {"EN": "Filter by date", "DE": "Nach Datum filtern", "AR": "تصفية حسب التاريخ"},
    "phh_from":                {"EN": "From",         "DE": "Von",           "AR": "من"},
    "phh_to":                  {"EN": "To",           "DE": "Bis",           "AR": "إلى"},
    "phh_col_sold_on":         {"EN": "Sold On",      "DE": "Verkauft am",   "AR": "تاريخ البيع"},
    "phh_col_brand":           {"EN": "Brand",        "DE": "Marke",         "AR": "الماركة"},
    "phh_col_model":           {"EN": "Model",        "DE": "Modell",        "AR": "الموديل"},
    "phh_col_storage":         {"EN": "Storage",      "DE": "Speicher",      "AR": "السعة"},
    "phh_col_imei":            {"EN": "IMEI",         "DE": "IMEI",          "AR": "IMEI"},
    "phh_col_sale_price":      {"EN": "Sale Price",   "DE": "Verkaufspreis", "AR": "سعر البيع"},
    "phh_col_note":            {"EN": "Note",         "DE": "Notiz",         "AR": "ملاحظة"},
    "phh_close":               {"EN": "Close",        "DE": "Schließen",     "AR": "إغلاق"},
    "phh_summary":             {
        "EN": "{n} phone(s) sold — total €{total}",
        "DE": "{n} Telefon(e) verkauft — Gesamt €{total}",
        "AR": "تم بيع {n} هاتف — الإجمالي €{total}",
    },

    # ── Phone transaction operation labels ───────────────────────────────────
    "pht_op_create":           {"EN": "Added",        "DE": "Hinzugefügt",   "AR": "تمت الإضافة"},
    "pht_op_edit":             {"EN": "Edited",       "DE": "Bearbeitet",    "AR": "تم التعديل"},
    "pht_op_sold":             {"EN": "Sold",         "DE": "Verkauft",      "AR": "مباع"},
    "pht_op_reserved":         {"EN": "Reserved",     "DE": "Reserviert",    "AR": "محجوز"},
    "pht_op_in_stock":         {"EN": "Returned to Stock", "DE": "Zurück auf Lager", "AR": "أُعيد إلى المخزون"},
    "pht_op_delete":           {"EN": "Deleted",      "DE": "Gelöscht",      "AR": "تم الحذف"},

    # ── Shop settings — Modules card ─────────────────────────────────────────
    "shop_card_modules":       {"EN": "Modules",      "DE": "Module",        "AR": "الوحدات"},
    "shop_card_modules_desc":  {"EN": "Enable optional modules for this shop. Requires app restart.", "DE": "Optionale Module für diesen Shop aktivieren. Erfordert einen Neustart der App.", "AR": "تفعيل الوحدات الاختيارية لهذا المتجر. يتطلب إعادة تشغيل التطبيق."},
    "shop_module_phones_label":{"EN": "Phone inventory (IMEI tracking)", "DE": "Telefonbestand (IMEI-Verfolgung)", "AR": "مخزون الهواتف (تتبع IMEI)"},
    "shop_module_phones_tip":  {
        "EN": "Adds the 📱 Phones tab — track individual phone units by IMEI,\nstorage, condition, battery %, buy/sell price, and barcode labels.\nIntended for phone-shop customers. Requires app restart.",
        "DE": "Fügt den Tab 📱 Telefone hinzu — verfolgt einzelne Telefoneinheiten nach IMEI,\nSpeicher, Zustand, Akku %, Kauf-/Verkaufspreis und Barcode-Etiketten.\nFür Telefonshop-Kunden gedacht. Erfordert einen Neustart der App.",
        "AR": "يضيف تبويب 📱 الهواتف — لتتبع كل وحدة هاتف برقم IMEI،\nالسعة، الحالة، نسبة البطارية، سعر الشراء/البيع، وملصقات الباركود.\nمخصص لعملاء محلات الهواتف. يتطلب إعادة تشغيل التطبيق.",
    },
    "shop_modules_changed_title": {"EN": "Modules changed", "DE": "Module geändert", "AR": "تم تغيير الوحدات"},
    "shop_modules_changed_body":  {
        "EN": "Module changes (Phones tab) will take effect after restarting the application.",
