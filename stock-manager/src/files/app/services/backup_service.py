...
Auto-backup support: should_backup_now(), auto_backup(), get_last_backup_time().
"""
...
class BackupService:
    ...
    def get_last_backup_time(self) -> datetime | None:
        """Return datetime of the last successful auto-backup (from app_config),
        or fall back to the modification time of the newest backup file."""
        # First try app_config record
        try:
            with get_connection() as conn:
                row = conn.execute(
                    "SELECT value FROM app_config WHERE key='last_auto_backup'"
                ).fetchone()
                if row and row["value"]:
                    return datetime.fromisoformat(row["value"])
        except Exception:
            pass
        # Fall back to most recent backup file mtime
        backups = self.list_backups()
        if backups:
            return datetime.fromtimestamp(backups[0]["date"])
        return None

    def _record_backup_time(self, dt: datetime | None = None) -> None:
        """Persist the backup timestamp to app_config."""
        ts = (dt or datetime.now()).isoformat(timespec="seconds")
        try:
            with get_connection() as conn:
                conn.execute(
                    "INSERT OR REPLACE INTO app_config (key, value) VALUES ('last_auto_backup', ?)",
                    (ts,),
                )
        except Exception:
            pass

    def should_backup_now(self, interval_hours: int = 24) -> bool:
        """True if more than `interval_hours` have passed since the last backup."""
        last = self.get_last_backup_time()
        if last is None:
            return True
        return datetime.now() - last >= timedelta(hours=interval_hours)

    def auto_backup(self, retain: int = 10, backup_dir: str | None = None) -> str:
        """Create a backup and rotate old ones. Returns path of new backup."""
        path = self.create_backup(dest_dir=backup_dir or None)
        self._record_backup_time()
        self.auto_cleanup(keep=retain)
        return path
