...
class BarcodeGenService:
    ...
    def generate_for_phones(self, phones) -> list[BarcodeEntry]:
        """Build BarcodeEntry list for phone units.

        Barcode text = IMEI directly (15 digits, Code 128 numeric submode
        encodes them in ~28mm — comfortably fits any sticker). For units
        without an IMEI the code is ``PHN{id:05d}`` (e.g. ``PHN00042``).

        No DE-keyboard transforms are applied: IMEI codes are pure digits
        (no dashes, no letters that swap under QWERTZ), and the PHN prefix
        uses only uppercase ASCII which Code 128 encodes without ambiguity.
        """
        entries: list[BarcodeEntry] = []
        for p in phones:
            bc = phone_barcode_text(p)
            label_parts = [p.model_brand, p.model_name]
            if p.storage:
                label_parts.append(p.storage)
            if p.condition:
                label_parts.append(p.condition_label)
            label = " · ".join(x for x in label_parts if x)
            entries.append(BarcodeEntry(
                item_id      = None,
                barcode_text = bc,
                db_text      = bc,   # digits / PHN-code need no ß transform
                display_label= label,
                brand        = p.model_brand,
                part_type    = p.storage or "",
                color        = p.condition_label,
            ))
        return entries

    def export_phones_for_yunprint(
        self,
        phones,
        output_path: str,
        *,
        ...
        """Write a YunPrint **.txt** CSV for phone unit labels.

        Columns:
          barcode     — IMEI (15 digits) or PHN-code for IMEI-less units
          model       — brand + model name, e.g. "Apple iPhone 15 Pro"
          storage     — "128GB"
          condition   — "New" / "Used" / "Refurbished"
          battery     — "85%" or "-"
          sell_price  — "€350.00" or "-"
          imei        — full IMEI or "-"
          label       — combined display string

        Returns the path written.  Raises BarcodeValidationError if any
        code would not scan off the sticker (only when validate=True).
        """
        import csv as _csv
        import os as _os

        entries = self.generate_for_phones(phones)
        ...
        cfg_sym = "€"
        try:
            from app.core.config import ShopConfig as _SC
            _cfg = _SC.get()
            cfg_sym = getattr(_cfg, "currency_symbol", "€") or "€"
        except Exception:
            pass

        headers = ["barcode", "model", "storage", "condition",
        ...
        with open(output_path, "w", encoding="utf-8-sig", newline="") as f:
            w = _csv.writer(f, quoting=_csv.QUOTE_MINIMAL)
            w.writerow(headers)
            for p, e in zip(phones, entries):
                price = (f"{cfg_sym}{p.sell_price:.2f}"
                         if p.sell_price is not None else "-")
                batt  = f"{p.battery_pct}%" if p.battery_pct is not None else "-"
                w.writerow([
                    _clean(e.barcode_text),
                    _clean(f"{p.model_brand} {p.model_name}".strip()),
                    _clean(p.storage or "-"),
                    _clean(p.condition_label),
                    _clean(batt),
                    _clean(price),
                    _clean(p.imei or "-"),
                    _clean(e.display_label),
                ])
                written += 1
        return output_path


# ── Phone barcode text (module-level helper, importable by phone_repo etc.) ──

def phone_barcode_text(phone) -> str:
    """Return the printable barcode string for one phone unit.

    Priority:
      1. IMEI (15 digits) — the industry-standard identifier; Code 128
         encodes 15 digits in ~28 mm, well within any sticker.
      2. ``PHN{id:05d}`` — short generated code for units without IMEI
         (e.g. ``PHN00042``).  All uppercase alphanumeric — safe for
         both Code 128 and Code 39, no German-keyboard ambiguity.
    """
    if phone.imei:
        return phone.imei.strip()
    return f"PHN{phone.id:05d}"
